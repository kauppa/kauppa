...
class TestCouponRepository: XCTestCase {
    ...
    func testCouponCreation() {
        ...
        let coupon = try! repository.createCoupon(with: data)
    ...
    func testCouponUpdate() {
        ...
        data.note = "foobar"
        let updatedCoupon = try! repository.updateCoupon(for: coupon.id, with: data)
    ...
    func testStoreCalls() {
        ...
        let _ = try? repository.getCoupon(for: coupon.id)
        XCTAssertTrue(store.getCalled)  // this should've called the store
        repository.codes = [:]      // clear the codes
        let _ = try? repository.getCoupon(for: coupon.data.code!)
